...
[@react-three/fiber](https://github.com/pmndrs/react-three-fiber) on Mobile with NativeScript.  
Declaritive three.js with the react-three-fiber reconciler running on NativeScript Canvas.
...
- **Android** (physical device): renders. The emulator is not a usable target — its software GPU exposes no WebGPU, so the renderer falls back to WebGL2 and the native canvas layer aborts (`SIGABRT` out of Mesa virtio-gpu). On real hardware WebGPU is present and the scene renders. `ns build android` produces the APK regardless of target.
