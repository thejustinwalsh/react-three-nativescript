...
if (isAndroid) {
  debug(
    '[R3FNS] Android detected via @nativescript/core isAndroid - calling Helpers.initialize() before canvas-polyfill',
  )
  Helpers.initialize()
} else {
  debug('[R3FNS] Not Android (per @nativescript/core) - skipping Helpers.initialize()')
}
...
function patchOneGLTFLoader(ctor: any, tag = '[R3FNS MESHOPT]'): boolean {
  ...
  if (typeof set === 'function') {
    ...
    debug(`${tag} Patched setMeshoptDecoder on`, ctor.name || 'GLTFLoader')
    return true
  }
  return false
}
...
function setupThreeGLTFLoaderInterceptor(): void {
  ...
  try {
    ...
    debug('[R3FNS MESHOPT] Installed assignment interceptor on global THREE.GLTFLoader for early auto-patch')
  } catch {
...
function installDecoderRedirect(): void {
  const tag = '[R3FNS DECODERS]'
  debug(`${tag} installDecoderRedirect() called at startup`)
  try {
    if (typeof Folder?.exists !== 'function') {
      debug(`${tag} Folder API not available, skipping decoder redirect`)
      return
    }
    const appPath = knownFolders.currentApp().path
    const dir = nsPath.join(appPath, 'assets', 'decoders')
    debug(`${tag} Checking for decoder directory: ${dir}`)
    const exists = Folder.exists(dir)
    debug(`${tag} Folder.exists("${dir}") = ${exists}`)
    if (!exists) {
      debug(`${tag} No decoders directory found — no redirect installed`)
      return
    }
    ...
    debug(`${tag} Found ${entities.length} entities in decoder dir:`)
    for (const entity of entities) {
      debug(`${tag}   - ${entity.name} (path: ${entity.path})`)
      names.add(entity.name)
    }
    if (!names.size) {
      debug(`${tag} Decoder directory exists but is empty`)
      return
    }
    debug(`${tag} Will redirect these decoder filenames:`, Array.from(names))
    ...
    if (typeof origFetch === 'function') {
      debug(`${tag} Patching global fetch for decoder redirect`)
    ...
    if (XHR?.prototype && typeof XHR.prototype.open === 'function') {
      debug(`${tag} Patching XMLHttpRequest.prototype.open for decoder redirect`)
    ...
    debug(`${tag} Decoder redirect installation complete`)
  } catch (err) {
...
debug('[R3FNS POLYFILLS] polyfills.ts evaluating - about to call installDecoderRedirect()')
...
export function patchGLTFLoader(): void {
  ...
  patchOneGLTFLoader(GLTFLoader, '[R3FNS MESHOPT direct]')
  ...
  if (threeNs?.GLTFLoader && threeNs.GLTFLoader !== GLTFLoader) {
    patchOneGLTFLoader(threeNs.GLTFLoader, '[R3FNS MESHOPT global]')
  }
